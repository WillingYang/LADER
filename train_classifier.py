...
def main() -> int:
    ...
    model = QwenVLForFakeNewsClassification(
        ...
        config=ClassifierConfig(
            ...
            layer_expert_count=int(config["head"].get("layer_expert_count", 4)),
            layer_top_k=int(config["head"].get("layer_top_k", 2)),
            layer_router_hidden_dim=int(config["head"].get("layer_router_hidden_dim", 512)),
            layer_router_dropout=float(config["head"].get("layer_router_dropout", 0.1)),
            layer_router_temperature=float(config["head"].get("layer_router_temperature", 1.0)),
            layer_teacher_temperature=float(config["head"].get("layer_teacher_temperature", 1.0)),
            layer_decay=float(config["head"].get("layer_decay", 0.5)),
            layer_expert_aux_weight=float(config["head"].get("layer_expert_aux_weight", 0.2)),
            layer_router_loss_weight=float(config["head"].get("layer_router_loss_weight", 0.2)),
            residual_gate_hidden_dim=int(config["head"].get("residual_gate_hidden_dim", 512)),
